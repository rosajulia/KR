...
def main():
  ...
  parser.add_argument("heuristic", type=int, help="SAT strategies: 1; random, 2;\
    JW one-sided, 3; JW two-sided, 4; DLCS, 5; DLIS [default: 1]", default = 1)
  parser.add_argument("input1", type=argparse.FileType("r", encoding="UTF-8"),\
    help="Filepath rules for sudoku", required=True)
  parser.add_argument("input2", type=argparse.FileType("r", encoding="UTF-8"),\
    help="Filepath Sudoku file", required=False)    
  args = parser.parse_args()

  print("Thanks for using our SAT solver!\nYou chose strategy {}\nInputfile:\
    \n{}\n{}".format(args.heuristic, args.input1.name, args.input2.name))
  
  # Read dimacs files and print stats 
  # Source: https://github.com/mxklabs/mxklabs-python/tree/master/mxklabs/dimacs
  ...
  # write output to csv for statistical analysis
...
if __name__ == "__main__":
  # global variables to keep track of algorithm metrics
  global splits
  splits = []
